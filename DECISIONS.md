...
## v1.3 — image-first pass (2026-09-21)

Round three of the design brief, after "I really don't like the design" and
"I want more visuals". The correction that mattered: the first two passes
added **information** design — badges, counts, a progress ring, a pipeline —
when the ask was **imagery that makes the tool easier to use**. Recognition
over recall: you should find a shot by seeing it, not by reading a code.

- 2026-09-21 — **The Shots grid is a contact sheet, and the sheet is the default view.** The old card wrapped a small frame in three bands of chrome (slate strip with the code, image, footer), so the picture was an attachment to a record. The frame is now the whole card: a 2px status edge on the frame itself, a status dot, and one caption that leads with the human title and demotes the mono code to the fallback reference. Nothing sits bottom-left over the image — that is where a still carries its own burned-in slate, and two pieces of text in one corner are unreadable (caught on the seeded frames, which burn the code in exactly there). Consequence: with no slate strip above it, an empty frame no longer duplicates the code, so it carries its own label again. Three specs asserted on `tbody tr` or the table's row controls while silently relying on the table being default; they now call `showShotsTable()`.
- 2026-09-21 — **One page frame for every screen; headings no longer move.** Pages each picked their own width — 2xl on the wizard, 3xl on a QC run, 4xl on Team, 5xl on Files, 6xl on Shots — all `mx-auto` CENTRED, and in two different type sizes. So the heading landed at a different x (and y) on every screen and appeared to jump as you moved through the rail; worse, the Shots heading slid sideways when you toggled table/sheet, because the two views cap at different widths. `PageShell` + `PageHeader` left-align every page against one gutter: a page may still cap its own measure, but that cap grows to the RIGHT. `meta` renders INSIDE the `h1` with a literal space — accessible names join text nodes verbatim, so a margin alone announces "Characters· 3".
- 2026-09-21 — **Characters left the "Pre-production" section.** One item under a shouted mono section heading read as filing-cabinet structure; a character is a thing you make, like a shot, so it sits with Board and Shots.
- 2026-09-21 — **The rail is the person's, not the product's: reorder, hide, collapse.** "Customise menu" turns on drag handles and per-item hide; the rail also collapses to icons. Order is stored **by href, not index**, so adding or removing a destination in a later release cannot scramble a saved arrangement — unknown hrefs are ignored and new ones appear in their default position. Hiding is only honoured outside edit mode, since you have to see an item to bring it back.
- 2026-09-21 — **Up to five favourites in the top bar, set with a star beside any page heading.** Favourites key on the full path INCLUDING the query, so `/shots?status=rework` pins separately from `/shots` and someone can park the filtered view they live in. Starring a sixth replaces the oldest rather than silently failing. Stored per device in localStorage next to the theme, the shots view and the rail prefs — a personal convenience, no schema and no sync to reason about; the trade is that none of it follows you to another machine. Moving these to Convex is a deliberate later decision, not a default.

## v1.4 — help inside the product (2026-09-21)

- 2026-09-21 — **The manual moved into the app; docs/MANUAL.md became a pointer.** v1.2 shipped the manual as Markdown in the repo, which meant the people it was written for — artists and producers — never saw it, and it drifted from the product the moment anything changed. Help is now two layers, both in the product: a drawer keyed to the current route (`?` or the Help button) and the illustrated walkthrough at `/help`. `docs/MANUAL.md` now says where the manual lives, which file to edit for each part, and how to regenerate the screenshots; deleting the prose copy also took 2.9MB of duplicated images out of git.
- 2026-09-21 — **Per-screen help is route-keyed data, not prose.** `lib/help.ts` holds one entry per route — purpose, ordered steps, and the thing people get wrong — matched most-specific-first with opaque id segments normalised to `:id`, so `/p/<id>/review/<id>` resolves to `/p/:id/review/:id`. Written as short imperative steps because someone opening help is stuck, not settling in to read. Consequence: adding a screen means adding a HELP_TOPICS entry, and the drawer degrades to the process tab when a route has none.
- 2026-09-21 — **One shortcut list; `keyboard-overlay.tsx` deleted.** The list existed twice (the app shell's overlay and the Review Room's hints) and had already drifted: the overlay was missing the Review Room's zoom keys for a release. `components/app/shortcuts.ts` is now the single source, rendered by both the help drawer and the guide. `?` opens contextual help rather than a bare shortcut sheet — guidance is the more useful default and the keys are one tab away.
- 2026-09-21 — **The guide's screenshots are generated, not hand-shot.** `scripts/capture-help.mjs` captures them from the seeded demo into `public/help/`, and only the files `app/(app)/help/page.tsx` references, so an unused screenshot cannot rot unnoticed. The first set shipped with v1.2 was already stale — it predated the contact sheet, the aligned headings and the favourites bar — which is the argument for generating them rather than trusting anyone to remember.

## v1.5 — images on the screens that had none (2026-09-21)

- 2026-09-21 — **A production has a face: `listForStudio` returns a cover.** The home screen — the first thing anyone sees after signing in — carried no picture at all, in a tool for film. The cover is the best-ranked **decided** shot's thumbnail (delivered > final > approved > picked): a candidate nobody has chosen is not what the studio stands behind. Picked during the same bounded scan that already produces `shotCounts`, so it adds one document read and one storage URL per production and no second pass. Rejected letting a producer pin a cover: more control, but another thing to maintain, and on day one there is nothing to pin either way.
- 2026-09-21 — **The Overview opens on the work, not on a feed.** It led with "Today's activity" — true sentences ("Dara added v2 to SC120_SH010") that do not add up to a picture of the production. `<OverviewReel>` puts six frames across the top ("The film so far"), decided work first and in-flight behind it so the row is never empty on a production that has started; the feed moves below. This is an editorial call, not a layout one: a producer who reads that feed every morning will notice. The vitals band's own 64px "Picked" strip was removed in the same pass — two strips of the same thumbnails, one of them too small to read, was one too many.
- 2026-09-21 — **Text never sits over the bottom of a still.** Established on the contact sheet, and promptly broken again reaching for a poster look on Productions: a still carries its own burned-in slate or lower-third bottom-left, and two pieces of text in one corner are unreadable — the seeded covers demonstrate it exactly. Captions go BELOW the frame everywhere: Productions, the sheet, My work. Only a status dot, a decided check and an assignee avatar ride the picture, in the corners a still rarely uses.
- 2026-09-21 — **The favourites strip is permanent.** It rendered only once something was pinned, which meant the feature taught nobody it existed. It now always occupies its own bordered slot in the top bar and, when empty, says where pinned pages land.
